...
| `Link` | Optional | URL to the original forum thread; the explorer's "Auction link", and the link users follow from the **open-auctions** banner/section (below). Fill it in for any `Open` auction — it's the whole point of surfacing a live auction. |
