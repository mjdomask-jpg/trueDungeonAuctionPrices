...
| `Status` | Yes (filter) | Auction outcome: `Closed` (completed), `Failed` (did not fund), `Open` (in progress), `Pending` (announced, not yet started). **Derived, never typed** — `IF(outcome <> "", outcome, IF(closeDate = "", "Open", "Closed"))`, with `Failed` and `Pending` both arriving through the `outcome` input. Auction **counts and statistics** include only `Closed`. `Open` and `Pending` drive the live banner (Prices) and the open/upcoming section (Auction Data) — but which of the two an auction IS is decided by `openDate` against today, not by the stored label, so a pending auction becomes open on its date with no republish (`auctionPhase` in `lib/data.ts`). `Failed` is excluded from everything, which is why that value needed no site change (backlog DATA-6). |
| `outcome` | No | The input `Status` reads. Blank on an ordinary auction, `Failed` on one that did not fund. It exists so a failed auction can keep its row instead of being deleted — see `updating-the-data.md` § *Recording a failed auction*. |
| `targetFunding`, `augment*`, `fundingNoAugment`, `preorderTotal` | Indirectly | Back-office financials; the Analytics → Funding & Context views are built on them. `targetFunding`, `augmentedTotal`, `fundingNoAugment` and `preorderTotal` are **populated on all 289 rows in every season, 2022 included** — they are formulas, so they always compute. The `augment*` **inputs** are sparse by design in every season: blank means the auction had no augment, not missing data. (This row read "the 2022 season is blank throughout" until 2026-09-03; that was true before the 2022 backfill and had not been re-measured since.) |
