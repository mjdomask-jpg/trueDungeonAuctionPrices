...
| `Status` | Yes (filter) | Auction outcome: `Closed` (completed), `Failed` (did not fund), `Open` (in progress). **Derived, never typed** — `IF(outcome <> "", outcome, IF(closeDate = "", "Open", "Closed"))`. Auction **counts and statistics** include only `Closed`; `Open` rows drive the live open-auctions banner (Prices) and section (Auction Data); `Failed` is excluded from both, which is why a third value needed no site change (backlog DATA-6). |
