...
**Now:** `Trade 1` and `Trade 2` are real categories in `tokenGroups.csv` with a
group order. `Trade 3`, `Trade 4` and `Trade 5` appear in **no CSV at all**.
...
| Trade 4 | Wish Ring | `Premium` (group `8k exclusive`) |
...
**Now:** `Premium` category, group `8k exclusive`, **288 rows in `prices.csv`**
